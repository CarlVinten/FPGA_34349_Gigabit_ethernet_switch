...
vcom -reportprogress 300 -work work crossbarfifo.vhd
...
add wave -label " Preamble Cnt" sim:/test/DUT/preamble_cnt 
...
add wave -hex -label "Port0_sum_reg" sim:/test/DUT/fcs_generate(3)/u_fcs/sum_reg
add wave -hex -label "Port1_sum_reg" sim:/test/DUT/fcs_generate(2)/u_fcs/sum_reg
add wave -hex -label "Port2_sum_reg" sim:/test/DUT/fcs_generate(1)/u_fcs/sum_reg
add wave -hex -label "Port3_sum_reg" sim:/test/DUT/fcs_generate(0)/u_fcs/sum_reg

# 4. Monitor the FCS output signals [cite: 19, 26, 50]
add wave -divider "FCS Status"
add wave sim:/test/DUT/fcs_sof
add wave -label "Delay SOF(0)" sim:/test/DUT/fcs_generate(0)/u_fcs/delay_SOF 
add wave -label "Delay SOF(0)" sim:/test/DUT/fcs_generate(1)/u_fcs/delay_SOF 
add wave -label "Delay SOF(0)" sim:/test/DUT/fcs_generate(2)/u_fcs/delay_SOF 
add wave -label "Delay SOF(0)" sim:/test/DUT/fcs_generate(3)/u_fcs/delay_SOF 
...
add wave -hex sim:/test/DUT/mac_data_to_fsm 

add wave -divider "FIFO"
add wave -hex sim:/test/DUT/data_in_to_fifo 
add wave -hex sim:/test/DUT/data_out_to_fsm 


add wave -divider "Output CTRL FSM"
add wave -label "State2" sim:/test/DUT/state2 

add wave -label "EOF" sim:/test/DUT/eof_signal 
